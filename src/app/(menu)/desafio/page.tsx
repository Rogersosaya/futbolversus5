...
/** Desafío Individual — streak ladders (timed) and free training. */
export default function DesafioPage() {
  const [selected, setSelected] = useState<string | null>(null);
  const [mode, setMode] = useState<"timer" | "libre">("timer");
  const [trainLevel, setTrainLevel] = useState("l1");

  const cards = CHALLENGE_GAMES.map((g) => {
    const s = STREAKS[g.id];
    const al = activeTimerLevel(s);
    return (
      <button
        key={g.id}
      ...
      >
        <div className="mg-art" style={artStyle(g.gradient)}>
          ...
          <span className={`streak-badge${s.today ? " done" : ""}`}>
            {s.today ? "HOY ✓" : `🔥 ${al}`}
          </span>
        </div>
    ...
    <div className="section-head">
      <div>
        ...
        <div className="sh-sub">
          Sin rival, solo tú contra el juego. Forja tu racha en Contrarreloj o entrena libre.
        </div>
      </div>
    </div>
  );
  ...
  if (!selected) {
    ...
    const g = CHALLENGE_GAMES.find((x) => x.id === selected)!;
    const s = STREAKS[selected];
    const al = activeTimerLevel(s);
    ...
    detail = (
      <div className="mg-detail">
        <div className="dt-art" style={artStyle(g.gradient)}>
          <span className="ic">
            <Sym id={g.ic} />
          </span>
        </div>
  ...
  return (
    <div className={`amistoso${selected ? " has-sel" : ""}`}>
      {head}
